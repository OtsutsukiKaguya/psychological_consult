...
const fetchTutorInfo = async () => {
    if (!currentUser.value?.id) return
    try {
        const res = await axios.get(`${BASE_URL}/api/search/tutor/${currentUser.value.id}`)
        if (res.data.code === 0 && Array.isArray(res.data.data) && res.data.data.length > 0) {
            const tutorInfo = res.data.data[0]
            // 设置同时咨询人数
            if (tutorInfo.counselorSameTime) {
                maxConsultations.value = tutorInfo.counselorSameTime
            }
            // 设置标签
            if (tutorInfo.tag) {
                tags.value = tutorInfo.tag.split(',').map(tag => tag.trim()).filter(tag => tag)
            }
        }
    } catch (e) {
        console.error('获取督导基本信息失败:', e)
    }
}
...
const updateTags = async (newTags) => {
    ...
    try {
        ...
        const res = await axios.post(`${BASE_URL}/api/tutor/update-tag`, null, {
            params: {
...
const handleMaxConsultationsChange = async (newValue) => {
    ...
    try {
        const res = await axios.post(`${BASE_URL}/api/tutor/update-sametime`, null, {
            params: {
                id: currentUser.value.id,
