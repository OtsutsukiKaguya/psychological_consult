...
public interface ChatSessionRepository extends JpaRepository<ChatSession, String> {  // 修改主键类型为 String
    ...
    // 根据最后活动时间范围查找会话
    List<ChatSession> findByLastActivityAtBetween(LocalDateTime start, LocalDateTime end);  // 无需修改

    // 查找指定时间之后有活动的会话
    List<ChatSession> findByLastActivityAtAfter(LocalDateTime time);  // 无需修改
    ...
    List<ChatSession> findRecentActiveSessionsByUserId(@Param("userId") String userId, @Param("limit") int limit);  // 修改用户ID为 String 类型
}
